import {
  ...
  TrashIcon,
  PencilIcon,
  FilePenLineIcon as FilePenlineIcon,
  XIcon,
  UploadCloud,
  LoaderCircle,
} from "lucide-react";
...
const Dashboard = () => {
  ...
  const createNewResume = async (event) => {
    try {
  ...
  const uploadResume = async (event) => {
    ...
    try {
      // Just create a new resume with the given title, skipping AI text extraction entirely
      const { data } = await api.post(
        "/api/resumes/create",
        { title },
        {
  ...
  const editTitle = async (event) => {
    try {
      ...
      setAllResumes(
        allResumes.map((resume) => (resume._id === editResumeId ? data.resume : resume)),
      );
      setTitle("");
      setEditResumeId("");
          ...
          >
            ...
            >
              ...
              <input
                ...
                type="text"
                placeholder="Enter resume title"
                className="w-full px-4 py-2 mb-4 focus:border-green-600 ring-green-600 outline-none border rounded-md"
                required
              />
              <div className="mb-4">
                <label
